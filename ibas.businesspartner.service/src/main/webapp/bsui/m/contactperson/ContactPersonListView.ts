...
export class ContactPersonListView extends ibas.BOListView implements IContactPersonListView {
            ...
            attributes: [
                new sap.m.ObjectAttribute("", {
                    ...
                    formatter(data: any): any {
                        return ibas.i18n.prop("bo_contactperson_position") + ":" + data;
                    }
                }),
                new sap.m.ObjectAttribute("", {
                }).bindProperty("text", {
                    path: "Telephone1",
                    formatter(data: any): any {
                        return ibas.i18n.prop("bo_contactperson_telephone1") + ":" + data;
                    }
                }),
            ],
        });
    ...
    showData(datas: bo.ContactPerson[]): void {
        ...
        if (!ibas.objects.isNull(model)) {
            // 已存在绑定数据，添加新的
            let hDatas: bo.ContactPerson[] = (<any>model).getData();
