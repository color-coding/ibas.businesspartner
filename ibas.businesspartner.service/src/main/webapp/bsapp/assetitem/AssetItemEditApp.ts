...
namespace businesspartner {
    export namespace app {
        ...
        export class AssetItemEditApp extends ibas.BOEditApplication<IAssetItemEditView, bo.AssetItem> {
            ...
            /** 应用名称 */
            static APPLICATION_NAME: string = "businesspartner_app_assetitem_edit";
            /** 业务对象编码 */
            ...
            protected viewShowed(): void {
                ...
                if (ibas.objects.isNull(this.editData)) {
                    // 创建编辑对象实例
                    this.editData = new bo.AssetItem();
                    this.proceeding(ibas.emMessageType.WARNING, ibas.i18n.prop("shell_data_created_new"));
                }
                this.view.showAssetItem(this.editData);
            }
            run(): void;
            ...
            run(): void {
                ...
                if (ibas.objects.instanceOf(arguments[0], bo.AssetItem)) {
                    let data: bo.AssetItem = arguments[0];
                    // 新对象直接编辑
                    if (data.isNew) {
                        that.editData = data;
                        that.show();
                    ...
                    if (!ibas.objects.isNull(criteria) && criteria.conditions.length > 0) {
                        ...
                        boRepository.fetchAssetItem({
                            ...
                            onCompleted(opRslt: ibas.IOperationResult<bo.AssetItem>): void {
                                let data: bo.AssetItem;
                                if (opRslt.resultCode === 0) {
                                    data = opRslt.resultObjects.firstOrDefault();
                                }
                                if (ibas.objects.instanceOf(data, bo.AssetItem)) {
                                    // 查询到了有效数据
                                    that.editData = data;
                                    that.show();
                                } else {
            ...
            protected createData(clone: boolean): void {
                ...
                let createData: Function = function (): void {
                    if (clone) {
                        ...
                        // 新建对象
                        that.editData = new bo.AssetItem();
        ...
        /** 视图-资产项目 */
        export interface IAssetItemEditView extends ibas.IBOEditView {
            /** 显示数据 */
