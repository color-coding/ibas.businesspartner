 ...
 namespace businesspartner {
    export namespace app {
        ...
        export class PaymentTermEditApp extends ibas.BOEditApplication<IPaymentTermEditView, bo.PaymentTerm> {
            ...
            /** 应用名称 */
            static APPLICATION_NAME: string = "businesspartner_app_paymentterm_edit";
            /** 业务对象编码 */
            ...
            protected viewShowed(): void {
                ...
                if (ibas.objects.isNull(this.editData)) {
                    // 创建编辑对象实例
                    this.editData = new bo.PaymentTerm();
                    this.proceeding(ibas.emMessageType.WARNING, ibas.i18n.prop("shell_data_created_new"));
                }
                this.view.showPaymentTerm(this.editData);
            }
            run(): void;
            ...
            run(): void {
                ...
                if (ibas.objects.instanceOf(arguments[0], bo.PaymentTerm)) {
                    let data: bo.PaymentTerm = arguments[0];
                    // 新对象直接编辑
                    if (data.isNew) {
                        that.editData = data;
                        that.show();
                    ...
                    if (!ibas.objects.isNull(criteria) && criteria.conditions.length > 0) {
                        ...
                        boRepository.fetchPaymentTerm({
                            ...
                            onCompleted(opRslt: ibas.IOperationResult<bo.PaymentTerm>): void {
                                let data: bo.PaymentTerm;
                                if (opRslt.resultCode === 0) {
                                    data = opRslt.resultObjects.firstOrDefault();
                                }
                                if (ibas.objects.instanceOf(data, bo.PaymentTerm)) {
                                    // 查询到了有效数据
                                    that.editData = data;
                                    that.show();
                                } else {
            ...
            protected createData(clone: boolean): void {
                ...
                let createData: Function = function (): void {
                    if (clone) {
                        ...
                        // 新建对象
                        that.editData = new bo.PaymentTerm();
        ...
        /** 视图-付款条款 */
        export interface IPaymentTermEditView extends ibas.IBOEditView {
            /** 显示数据 */
