 ...
 *
 */
@LogicContract(IBusinessPartnerAssetIncreasesContract.class)
public class BusinessPartnerAssetIncreasesService
		extends BusinessPartnerAssetLogic<IBusinessPartnerAssetIncreasesContract, IBusinessPartnerAssetJournal> {

	@Override
	protected IBusinessPartnerAssetJournal fetchBeAffected(IBusinessPartnerAssetIncreasesContract contract) {
		// 检查单位是否一致
		...
		if (businessPartnerAssetJournal == null) {
			...
			businessPartnerAssetJournal.setDirection(emDirection.IN);
		}
	...
	@Override
	protected void revoke(IBusinessPartnerAssetIncreasesContract contract) {
		if (!this.getBeAffected().isDeleted()) {
